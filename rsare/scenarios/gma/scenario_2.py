...
class Scenario2(Scenario):
    ...
    def oracle_solution(self, run_oracle=False):
        ...
        if run_oracle:
            response7 = self.sarTools.load_AIS_data(**args7)
            print(response7)
        step7 = WorkflowStep(
            name="step7",
            op_type="WRITE",
        ...
        args8 = {'matching_score_threshold': 7.4e-6}
        if run_oracle:
            response8 = self.sarTools.sar_ais_matching(**args8)
            print(response8)
        step8 = WorkflowStep(
            name="step8",
            op_type="WRITE",
...
if __name__ == "__main__":
    ...
    sarTools = SARTools()
    scenario = Scenario2(scenario_id=2, scenario_input={}, sarTools=sarTools)
