...
from .constants import VESSEL_DETECTIONS_DIR, INFRASTRUCTURE_DETECTIONS_DIR
...
class SARTools(App):
    ...
    def vessel_CFAR_detection(
            self,
            inner_window_size: Tuple[int, int] = (200, 200),
            outer_window_size: Tuple[int, int] = (600, 600),
        ...
        """
            ...
            Args:
                inner_window_size: (width, height) in pixels
                outer_window_size: (width, height) in pixels
    ...
    @type_check
    @event_registered(operation_type=OperationType.WRITE)
    def vessel_presence_length_estimation(self, tile_size: Tuple[int, int] = (80, 80)) -> str:
        """Estimate vessel presence probability and vessel length.

        Args:
            tile_size: (width, height) in pixels for downstream tiling / model input.
    ...
    def vessel_presence_length_estimation_hyperparameters(
            self,
            tile_size: Tuple[int, int] = (80, 80),
    ) -> dict:
        """Evaluate hyperparameters for vessel presence/length estimation.

        Args:
            tile_size: (width, height) in pixels used for the model input tile.
    ...
    def infrastructure_CFAR_detection(
            self,
            inner_window_size: Tuple[int, int] = (140, 140),
            outer_window_size: Tuple[int, int] = (200, 200),
    ) -> str:
        """
            Run CFAR infrastructure detection (loads from CSV).
    ...
    def infrastructure_CFAR_detection_hyperparameters(
        ...
        """Evaluate hyperparameters for CFAR-based offshore infrastructure detection.
            ...
            Args:
                inner_window_size: (width, height) in pixels for the inner CFAR ring.
                outer_window_size: (width, height) in pixels for the outer CFAR ring.
    ...
    def infrastructure_classification_hyperparameters(
        ...
        Args:
            tile_size: (width, height) in pixels used for the model input tile.
            multiband_image_bands: list of availabe bands to concat (S1A, S1B, RGB, NIR) (e.g., ["S1A",
                "S1B", "RGB", "NIR"])
        ...
        expected_bands = ["S1A", "S1B", "RGB", "NIR"]
    ...
    def fishing_nonfishing_classification(
        ...
        Args:
            tile_size: (width, height) in pixels for the environmental tiles (default: (100, 100)).
                      This should match the tile size used in generate_multiband_raster_stacks.
